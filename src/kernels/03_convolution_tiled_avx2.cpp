...
{
    const int TILE_W = 256; // Fits comfortably in L3 cache
    const __m256 v_inv_area = _mm256_set1_ps(inv_area);

#pragma omp parallel
    {
        // Thread-local buffers to avoid DRAM spills
        // Size: TILE_W * (height + K - 1)
...
#pragma omp for schedule(dynamic)
        for (int tile_x = 0; tile_x < width; tile_x += TILE_W)
        {
            ...
            // --- Step 1: Horizontal Pass for this Strip ---
            // Only compute the horizontal sums for the columns in this tile.
            for (int y = 0; y < height + K - 1; ++y)
            {
                float h_acc = 0.0f;
                const uint8_t *row_in = &padded[y * Wp + tile_x];

                // Initialize horizontal sliding window for this row
            ...
            // --- Step 2: Vertical SIMD Pass for this Strip ---
            // Initialize vertical accumulators
            ...
            // Slide vertically and demote directly to output_data
